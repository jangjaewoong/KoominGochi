...
class KoominGochi():
    ...
    def __init__(self):
        ...
        self.label3 = tkinter.Label(self.window, text="안녕!", bg="white", compound="top", font=font1)
    ...
    def btn_clicked(self, button):
        ...
        if not self.act.Gameover():
            ...
            self.reLabel.place(x=130, y=120)

            reKoomin = tkinter.PhotoImage(file='REkoomin.png')

            self.REKo = tkinter.Label(self.window, image=reKoomin)
            self.REKo.place(x=160, y=170)
            self.REKo.image = reKoomin

            self.reButton = tkinter.Button(self.window, text='재시작', width=20, height=3, command= lambda: self.btn_clicked("Retry"))
            self.reButton.place(x=150, y=500)
            self.quitButton = tkinter.Button(self.window, text='끝내기', width=20, height=3, command= lambda :self.btn_clicked("Quit"))
            self.quitButton.place(x=315, y=500)
    ...
    def selecttext(self, n):
        club_text=["나도 쿠순이랑 친해지고 싶다" , "스트레스가 확 풀려!!", "10시에 지세 앞으로?? 오케이" , "오늘 술집은 어디로 가나요?ㅎㅎ"
                , "역시 대학 Life는 동아리지!", "선배! 밥 사주세요~", "이게 대학생활이지~크으~", "오늘은 회식이다!",
                  "실례가 안 된다면 밥 좀 사 주십쇼 선배!",  "신규 부원 모집~~", "동방에 사람 많나요??","오늘 회식은 주당끼리에서 할까요?" ]
        cls_text=["이번 과제 꽤 어려운걸", "교수님 살려주세요", "할만한걸??" , "하.. 불태웠다..", "아 또!!! Error!!", "코딩은 그저 내 손이 해줄뿐 믿고 따라가 버렷!"
                    , "어? 이게 된다고?", "이...이게 왜 되는거여", "교수님 진도가 너무 빠릅니다", "아...집에 가고 싶다..."
                   ,"00101011001001001010110", "왜 컴파일이 안되냐고!!"]
        ...
        Pc_text=[ "서폿 야스오 한판 해볼까?", "이게 죽어?", "이게 맞아?", "소환사 협곡에 오신 여러분 환영합니다.", "삶의 낙은 역시 pc방에서 라면 먹는 거지!!!"
                , "아!!! 갱 좀 오라구!!!!", "우리 정글 뭐함?!?!", "아니! 이게 안 맞네?", "아아니! 이게 맞네?", "ㅈㄱㅊㅇ 오지네..", "남탓은 해도 된다.. 우린 남이니까.."
                  , "우리 서폿은 어디서 뭐해!!"]
        Mov_text=["범죄도시11 진짜 미쳤다!", "이야 아이언맨이 살아있었다고??", "이런 복선이 숨어 있었구만", "12월 14일 아바타 개봉!!!!!", "나랑 같이 영화 보러 갈래?"
                  ...
                  "혹시 오리지널 티켓 받을 수 있나요?", "이 영화관 팝콘 잘하네.. 냠냠", "내용은 좋은데 결말이 좀 아쉽네.."]
        ...
        selText = [club_text,cls_text,gym_text,Cu_text,gada_text,Pc_text,

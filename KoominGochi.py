...
class KoominGochi():
    ...
    def __init__(self):
        ...
        #쿠민이 이미지
        self.kookmin_image = tkinter.PhotoImage(file='../../Desktop/소프2/KoominGochi/sleep.png')
        self.label = tkinter.Label(self.window, image=self.kookmin_image)
        self.label.place(x=150, y=180)

        #말풍선
        text_image = tkinter.PhotoImage(file='../../Desktop/소프2/KoominGochi/text.png')
        label2 = tkinter.Label(self.window, image=text_image)
        label2.place(x=350, y=65)

        #능력치 이름바[지능, 매력, 돈, 외로움, 스트레스]
        button_intellect = tkinter.Button(self.window, text='지능',width=6, height=1)
        button_intellect.place(x=0, y=100)
        button_charm = tkinter.Button(self.window, text='매력',width=6, height=1)
        button_charm.place(x=0, y=130)
        button_money = tkinter.Button(self.window, text='돈',width=6, height=1)
        button_money.place(x=0, y=160)
        button_lonely = tkinter.Button(self.window, text='외로움',width=6, height=1)
        button_lonely.place(x=0, y=190)
        button_stress = tkinter.Button(self.window, text='스트레스',width=6, height=1)
        ...
        #날짜
        button_day = tkinter.Button(self.window, text=day.nextday(self),width=6)
